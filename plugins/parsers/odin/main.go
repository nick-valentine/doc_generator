...
func initRegexps() {
	...
	reProcDecl = regexp.MustCompile(`^` + sp + `*(` + w + `+)` + sp + `*::` + sp + `*(?:#` + w + `+` + sp + `+)*proc(?:` + sp + `|[\(\{\-]|$)`)
	reStructDecl = regexp.MustCompile(`^` + sp + `*(` + w + `+)` + sp + `*::` + sp + `*struct(?:` + sp + `|[\(\{\-]|$)`)
	reEnumDecl = regexp.MustCompile(`^` + sp + `*(` + w + `+)` + sp + `*::` + sp + `*enum(?:` + sp + `|[\(\{\-]|$)`)
	reUnionDecl = regexp.MustCompile(`^` + sp + `*(` + w + `+)` + sp + `*::` + sp + `*union(?:` + sp + `|[\(\{\-]|$)`)
	reBitSetDecl = regexp.MustCompile(`^` + sp + `*(` + w + `+)` + sp + `*::` + sp + `*bit_set(?:` + sp + `|[\[]|$)`)
	reInterfaceDecl = regexp.MustCompile(`^` + sp + `*(` + w + `+)` + sp + `*::` + sp + `*interface(?:` + sp + `|[\(\{\-]|$)`)
	reDistinctDecl = regexp.MustCompile(`^` + sp + `*(` + w + `+)` + sp + `*::` + sp + `*distinct` + sp + `+([^;\{\n]+)`)
